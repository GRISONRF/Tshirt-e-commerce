...
$(function(){
    ...
    //  this \/ is changing the var serch_params that already exists to show the values for the picked choices
    // crates a function because it is easier to call everything in the function, calls the itens in the function with the dot.
    function update_params()     {
        search_params.quantity = parseInt($("#quantity").val());
        search_params.color = $("#color .option-button.selected").attr('id'); //attr means returns attributes VALUE
        search_params.quality = $('#quality .option-button.selected').attr('id');
        search_params.style = $("#style").val();
        console.log(search_params);
        update_order_details();
    }
    //this \/ is only to print in the page what is the order/ the picked choices
 ...
 function calculate_total(){
        // needs to go inside the var produts to get the price. so: produtct > color > style > price
        // to get the color, i need to go to the other var 
